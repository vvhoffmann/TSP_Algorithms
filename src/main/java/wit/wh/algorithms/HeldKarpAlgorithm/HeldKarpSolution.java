 ...
 */
public class HeldKarpSolution extends TSPSolution {

    private static final double INF = Integer.MAX_VALUE;

    public HeldKarpSolution(ArrayList<Point> inputPoints) {
        super(inputPoints);
    }
    ...
    public ArrayList<Point> getTSPSolution() {
        ...
        if(n < PathUtils.MIN_POINTS_TO_RUN_ALGORITHM)
            return inputPoints;

        int N = 1 << n;
        double[][] dp = initializeDPTable(N, n);
        int[][] parent = new int[N][n];
        double[][] dist = PointUtils.getDistanceArray(inputPoints);
